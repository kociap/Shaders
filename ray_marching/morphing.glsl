float tri_lerp(float a, float b, float c, float t) {
 	return (1.0 - min(t, 1.0)) * a + (1.0 - abs(t - 1.0)) * b + max(t - 1.0, 0.0) * c;
}
...
float scene_sdf(vec3 point) {
    float moving_sphere = sphere_sdf(vec3(2.0 * sin(iTime / 1.4), 0, -1), 0.5, point);
    float sphere = sphere_sdf(vec3(0, 0, -1), 0.5, point);
    float sphere2 = sphere_sdf(vec3(0, 0, -1), 1.0, point);
    float cube = cube_sdf(vec3(0, 0, -1), 1.0, point);
    float freq = 4.0;
    float morph = tri_lerp(sphere, cube, sphere2, abs(mod(iTime, 2.0 * freq) - freq) / freq * 2.0);
    return smooth_min(morph, moving_sphere, 1.3);
}
