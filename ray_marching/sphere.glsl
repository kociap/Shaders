...
float scene_sdf(vec3 point) {
 	return sphere_sdf(vec3(0), 1.0, point);
}
...
void mainImage(out vec4 frag_color, in vec2 frag_coord) {
    ...
    float angle = iTime;
    vec3 light_pos = vec3(cos(angle), 0, sin(angle)) * 2.0;
    lights[0] = Point_Light(light_pos, vec3(1), 1.5);
    lights[1] = Point_Light(vec3(-1, -1, 2), vec3(1), 1.3);
